...
export function ForecastPageClient({
  ...
  function handleAddSuggestion(s: SuggestedRecurring) {
    const formData = new FormData();
    formData.set("type", s.type);
    formData.set("name", s.name);
    formData.set("amount", String(s.amount));
    formData.set("cadence", "monthly");
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <div>
          ...
          <p className="text-muted-foreground text-sm">
            Manage recurring items and view your financial projections
          </p>
        </div>
        <ScenarioToggle value={scenario} onChange={setScenario} />
      </div>

      {/* Auto-forecast suggestions banner */}
            ...
            <div className="flex items-start gap-3">
              ...
              <div className="flex-1">
                <p className="text-sm font-semibold">
                  Suggested recurring plan generated from your history
                </p>
                <p className="text-xs text-muted-foreground mt-1">
                  Based on your trailing 3-month average. Click to add any item,
                  then edit as needed.
                </p>
                  ...
                  >
                    <div>
                      ...
                      <p className="text-xs text-muted-foreground">
                        {item.cadence} · {item.category || "Uncategorized"}
                      </p>
                    </div>
      ...
      {showAddForm && (
        <Card>
          <CardHeader>
            <CardTitle className="text-base">Add Recurring Item</CardTitle>
          </CardHeader>
          <CardContent>
            <form
            ...
            >
              ...
              <div className="space-y-2">
                <Label htmlFor="category">
                  Category <span className="text-muted-foreground">(opt.)</span>
                </Label>
                <Select
                  id="category"
                  name="category"
      ...
      <Card>
        <CardHeader>
          <CardTitle className="text-base">
            Cash Balance Projection ({settings.horizonMonths} months)
          </CardTitle>
        </CardHeader>
        <CardContent>
          <CashflowChart
      ...
      <Card>
        <CardHeader>
          <CardTitle className="text-base">Monthly Projection Details</CardTitle>
        </CardHeader>
        <CardContent>
                ...
                {forecast.projections.map((p) => (
                  ...
                  >
                    ...
                    <td className="p-2.5 text-right text-success">
                      {formatCHF(p.projectedIncome)}
                    </td>
                    <td className="p-2.5 text-right text-danger">
                      {formatCHF(p.projectedExpenses)}
                    </td>
                    <td className="p-2.5 text-right text-warning">
                      {formatCHF(p.taxReserve)}
                    </td>
                    <td className="p-2.5 text-right font-medium">
                      {formatCHF(p.netCashflow)}
                    </td>
                    <td className="p-2.5 text-right font-bold">
                      {formatCHF(p.endingCash)}
                    </td>
                  </tr>
                ))}
              </tbody>
